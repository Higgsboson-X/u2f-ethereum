...
window.u2fServer = new window.web3.eth.Contract(compiledServer.abi);
...
window.confirmRegistration = async function() {
    ...
    .then((support) => {
        ...
        .then((res) => {
            ...
            u2fBind(record._appId, record._address, res)
            .then((dev) => {
                record._device = dev;
...
function u2fEnroll(appId, address) {
    ...
    return new Promise((resolve, reject) => {
        ...
        u2fServer.methods.enroll(appId).send({from: address})
        .then((receipt) => {
...
function u2fBind(appId, address, response) {
    ...
    return new Promise((resolve, reject) => {
        ...
        .then((challengeParameter) => {
            ...
            u2fServer.methods.bind(appId, challengeParameter, pubKey, keyHandle, certKey, signature).send({from: address})
            .then((receipt) => {
                console.log('[OK] ', receipt);
                details['transactionHash'] = receipt['transactionHash'];
            }, (error) => {
